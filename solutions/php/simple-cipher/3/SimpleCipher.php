...
class SimpleCipher
{
    ...
    {
        $this->key = $key;
        if(ctype_alpha($key) && ctype_lower($key))
        {
    ...
    public function encode(string $plainText): string
    {
        if(ctype_alpha($plainText) && ctype_lower($plainText))
        {
            ...
            for ($i=0; $i < count($characters); $i++)
            {
                ...
                if ($keys >= 26)
                {
                    return self::ALPHABETH[$keys-count(self::ALPHABETH)];
                }
                else
                {
                    return self::ALPHABETH[$keys];
                }
            }
        }
        else
    ...
    public function decode(string $cipherText): string
    {
        ...
        {    
            ...
            for ($i=0; $i < count($characters); $i++)
            {
                ...
                if ($key <= -1)
                {
                    return self::ALPHABETH[$key+count(self::ALPHABETH)];
                }
                else
                {
                    return self::ALPHABETH[$key];
                }
            }
        }
        else
