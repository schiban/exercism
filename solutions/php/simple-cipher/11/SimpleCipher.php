...
class SimpleCipher
{
    ...
    function __construct(string $key)
    {
        ...
        {
            $alphabeth = $this->key[0];
            if (str_contains($this->key, $alphabeth))
            {
                $this->key = array_search($this->key[0], self::ALPHABETH);
            }
            // Generates an array based on each character from the string $this->key
        ...
        else if ($this->key == "")
        {
            ...
            $this->key = array_search(self::ALPHABETH[$randomNumber], self::ALPHABETH);
        }
        else
        {
...
$stuff = new SimpleCipher('a');

echo $stuff->encode('abcdefghij');